        ...
        <ul class="space-y-4">
          @foreach($cartItems as $item)
            <li class="flex items-center gap-4">
              ...
              <img src="{{$item->product->image ? url('http://localhost/E-commerce/storage/app/private/photos/' . basename($item->product->image)) : asset('images/placeholder-image.jpg')}}" alt="product-image" class="rounded-2xl object-cover w-[150px] h-[150px]">
              <div>
                <h3 class="text-xl text-gray-900 font-bold">{{ $item->product->name }}</h3>
                <dl class="mt-0.5 space-y-px text-[10px] text-gray-600">
                  <div>
                    <dt class="inline font-medium">Size:</dt>
                    <dd class="inline font-medium">{{ $item->product->size }}</dd>
                  </div>
                  <div>
                    <dt class="inline font-medium">Color:</dt>
                    <dd class="inline font-medium">{{ $item->product->color }}</dd>
                  </div>
                </dl>
              </div>
